...
By default, the connector does not use keys for the Kafka messages it publishes. It can be configured to use the JMS message headers to set the key of the Kafka records. You could use this, for example, to use the MQMD correlation identifier as the partitioning key when the messages are published to Kafka. There are five valid values for the `mq.record.builder.key.header` that controls this behavior.
...
| JMSXGroupID                  | OPTIONAL_STRING | String    | org.apache.kafka.connect.storage.StringConverter       |
...
| `mq.jms.properties.copy.to.kafka.headers`       | Whether to copy JMS message properties to Kafka headers. When enabled, JMS properties maintain their original data types (Integer, Long, Short, Byte, Boolean, Float, Double, String). MQMD byte array properties (available when `mq.message.mqmd.read=true`) are always preserved as byte arrays. | boolean | false          |                                                                                                                                                                                     |
